	...
	namespace {
		...
		private:
			...
			[[nodiscard]] bool is_known_type(const SemanticType& t) const {
				...
				return is_builtin_type_name(t.name) || structs_.contains(t.name) || is_valid_template_instantiation(t.name);

			}



			[[nodiscard]] bool is_valid_template_instantiation(const std::string& name) const {

				size_t lt_pos = name.find('<');
				...
				auto it = structs_.find(base_name);
				...
				if (it->second.template_params.empty()) {
				...
				if (args.size() != it->second.template_params.size()) {
				...
				for (const auto& arg: args) {
					...
					bool is_valid_arg = is_builtin_type_name(arg) || structs_.contains(arg) || is_valid_template_instantiation(arg);
			...
			void check_statement(const Stmt* stmt, bool allow_yield, std::vector<SemanticType>* yielded) {
				...
				if (const auto* s = dynamic_cast<const VarDeclStmt*>(stmt)) {
					...
					bool is_valid_type = is_known_type(var_type) || is_valid_template_instantiation(var_type.name);

					if (!is_valid_type || (!is_builtin_type_name(var_type.name) && !is_visible_symbol(var_type.name) && !is_valid_template_instantiation(var_type.name))) {
			...
			SemanticType infer_expr_type(const Expr* expr) {
				...
				if (const auto* e = dynamic_cast<const NewExpr*>(expr)) {
					...
					if (!is_builtin_type_name(allocated.name) && !active_template_types_.contains(allocated.name)) {
				...
				if (const auto* e = dynamic_cast<const CallExpr*>(expr)) {
					...
					if (const auto* callee_id = dynamic_cast<const IdentifierExpr*>(e->callee.get())) {
						...
						if (structs_.contains(callee_id->name) && is_visible_symbol(callee_id->name)) {
							...
							// If callee has template_args, construct full type name

							if (!callee_id->template_args.empty()) {

								std::string full_name = callee_id->name + "<";

								for (size_t i = 0; i < callee_id->template_args.size(); ++i) {

									if (i > 0) full_name += ",";

									full_name += callee_id->template_args[i].name;
