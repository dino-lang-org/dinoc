...
namespace dino::codegen {
	...
	namespace {
		...
		std::optional<std::pair<std::string, std::vector<std::string>>> parse_template_instantiation(const std::string& name) {
		...
		private:
				...
				for (const auto& [path, unit]: *units_) {
					...
					for (const auto& decl: unit->declarations) {
						...
						if (const auto* struct_decl = dynamic_cast<const StructDecl*>(decl.get())) {
							...
							info.template_params = struct_decl->template_params;
			...
			llvm::Type* llvm_type(const SemanticType& type, bool decay_array = false) {
				...
				if (const auto parsed = parse_template_instantiation(type.name)) {
					...
					if (const auto base_found = structs_.find(base_name); base_found != structs_.end()) {

						const StructInfo& base_struct = base_found->second;

						// Check if this is a template struct

						if (!base_struct.template_params.empty() && base_struct.template_params.size() == args.size()) {

							// Create instantiated struct if it doesn't exist

							if (const auto inst_found = structs_.find(type.name); inst_found != structs_.end()) {

								return inst_found->second.llvm_type;

							}

							// Create new instantiated struct

							StructInfo inst_struct = base_struct;

							inst_struct.name = type.name;

							inst_struct.llvm_type = llvm::StructType::create(context_, type.name);

							inst_struct.is_template_instance = true;
