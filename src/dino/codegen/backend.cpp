		...
		private:
			...
			llvm::Constant* emit_constant_expression(const Expr* expr) {
				...
				if (const auto* literal = dynamic_cast<const LiteralExpr*>(expr)) {
					...
					if (literal->literal_kind == "KwNullptr") {
						return llvm::ConstantPointerNull::get(llvm::PointerType::get(context_, 0));
					}
			...
			void define_template_function_instances() {
				...
				while (changed && iterations < max_iterations) {
					...
					for (FunctionInfo* func: to_define) {
						...
						changed = true;
						if (func->kind == FunctionKind::Free) {
							define_free_function(*func);
						} else if (func->kind == FunctionKind::Method) {
							define_method(*func);
			...
			void define_constructor(FunctionInfo& info) {
				...
				// Apply in-struct field initializers for instance fields before constructor body.
				for (const auto& field: owner.fields) {
					if (field.decl == nullptr || field.decl->init == nullptr) {
						continue;
					}
					llvm::Value* init_value = emit_expression(field.decl->init.get());
					SemanticType init_type = infer_expr_type(field.decl->init.get());
					init_value = cast_value(init_value, init_type, field.type, true);
					if (init_value == nullptr) {
						continue;
					}
					llvm::Value* field_address = builder_.CreateStructGEP(owner.llvm_type, current_self_, field.index, field.name + ".ctor.init");
					builder_.CreateStore(init_value, field_address);
				}
			...
			llvm::Value* emit_lvalue(const Expr* expr) {
				...
				if (const auto* member = dynamic_cast<const MemberExpr*>(expr)) {
					...
					if (struct_info.llvm_type == nullptr || field->index >= struct_info.fields.size()) {
						errors_.push_back(std::format("Invalid field metadata for '{}.{}'", object_type.name, member->member));
						return nullptr;
					}
			...
			llvm::Value* emit_binary(const BinaryExpr& binary) {
				...
				if (binary.op == "==" || binary.op == "!=") {
					if (lhs_type.pointer_depth > 0 && rhs_type.name == "nullptr") {
						llvm::Value* rhs_null = llvm::ConstantPointerNull::get(llvm::cast<llvm::PointerType>(llvm_type(lhs_type)));
						return binary.op == "==" ? builder_.CreateICmpEQ(lhs, rhs_null, "ptr.eq.null")
												 : builder_.CreateICmpNE(lhs, rhs_null, "ptr.ne.null");
					}
					if (rhs_type.pointer_depth > 0 && lhs_type.name == "nullptr") {
						llvm::Value* lhs_null = llvm::ConstantPointerNull::get(llvm::cast<llvm::PointerType>(llvm_type(rhs_type)));
						return binary.op == "==" ? builder_.CreateICmpEQ(lhs_null, rhs, "ptr.eq.null")
												 : builder_.CreateICmpNE(lhs_null, rhs, "ptr.ne.null");
					}
				}
				...
				if (binary.op == "+" || binary.op == "-") {
					if (lhs_type.pointer_depth > 0 && is_numeric_type(rhs_type)) {
						...
						if (llvm_element_type == nullptr || llvm_element_type->isVoidTy()) {
							errors_.push_back(std::format("Invalid pointee type '{}' for pointer arithmetic", type_to_string(element_type)));
							return nullptr;
						}
				...
				if (binary.op == "==" || binary.op == "!=") {
					const bool lhs_ptr_like = lhs_type.pointer_depth > 0 || lhs_type.name == "nullptr";
					const bool rhs_ptr_like = rhs_type.pointer_depth > 0 || rhs_type.name == "nullptr";
					if (lhs_ptr_like && rhs_ptr_like) {
						SemanticType target_type = lhs_type.pointer_depth > 0 ? lhs_type : rhs_type;
						llvm::Value* lhs_ptr = cast_value(lhs, lhs_type, target_type, true);
						llvm::Value* rhs_ptr = cast_value(rhs, rhs_type, target_type, true);
						if (lhs_ptr == nullptr || rhs_ptr == nullptr) {
							return nullptr;
						}
						return binary.op == "=="
								   ? builder_.CreateICmpEQ(lhs_ptr, rhs_ptr, "ptr.eq")
			...
			llvm::Value* emit_new_expression(const NewExpr& expr) {
				...
				if (expr.is_array && new_arg_values.empty()) {
					// Plain array allocation (`new T[n]`) does not require element stores here.
					// This keeps codegen robust for dynamically-sized template allocations.
					return storage;
				}
			...
			llvm::Value* cast_value(llvm::Value* value, const SemanticType& from, const SemanticType& to, bool implicit) {
				...
				if (from.name == "nullptr" && to.pointer_depth > 0) {
					return llvm::ConstantPointerNull::get(llvm::cast<llvm::PointerType>(llvm_type(to)));
				}
				if (from.pointer_depth > 0 && to.pointer_depth > 0 &&
			...
			CallResolution resolve_call(const CallExpr* call) {
				...
				if (const auto* identifier = dynamic_cast<const IdentifierExpr*>(call->callee.get())) {
					...
					if (current_struct_ != nullptr && current_self_ != nullptr) {
						if (const auto methods = current_struct_->methods.find(identifier->name); methods != current_struct_->methods.end()) {
							...
							if (resolution.function != nullptr) {
								resolution.object_type = SemanticType{current_struct_->decl != nullptr ? current_struct_->decl->name : ""};
								resolution.object_address = current_self_;
								return resolution;
							}
						}
					}
			...
			void emit_var_decl(const VarDeclStmt& variable) {
				...
				if (variable.is_static) {
					...
					if (variable.is_array) {
						...
						init_value = cast_value(init_value, init_type, type, true);
						if (init_value != nullptr) {
							builder_.CreateStore(init_value, info.storage);
						}
					}
