...
namespace dino::frontend {
	namespace {
		...
		std::string describe_template_params(const std::vector<TemplateParam>& params) {
			if (params.empty()) {
				return {};
			}
		...
		for (const auto& decl: unit.declarations) {
			...
			if (const auto* fn = dynamic_cast<const FunctionDecl*>(decl.get())) {
				os << " name=" << fn->name << " returns=" << describe_type(fn->return_type)
				   << describe_template_params(fn->template_params) << describe_attributes(fn->attributes) << "\n";
				if (fn->body) {
					dump_stmt(fn->body.get(), os, 2);
				}
				continue;
			}
			if (const auto* global = dynamic_cast<const GlobalVarDecl*>(decl.get())) {
				os << " name=" << global->name << " type=" << describe_type(global->type);
				if (global->is_extern) {
					os << " #[extern]";
				}
				if (global->is_array) {
				...
				os << "\n";
				if (global->init) {
					dump_expr(global->init.get(), os, 2);
				}
				for (const auto& expr: global->array_init) {
					dump_expr(expr.get(), os, 2);
				}
				continue;
			}
			if (const auto* st = dynamic_cast<const StructDecl*>(decl.get())) {
				os << " name=" << st->name << describe_template_params(st->template_params) << "\n";
